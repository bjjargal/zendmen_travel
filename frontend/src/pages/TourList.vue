<template>
	<div class="min-h-screen">
		<a-page-header class="!p-0 mb-4 px-4 py-3 sticky top-0 z-10" title="Tour">
			<template #extra>
				<a-button type="primary" @click="open = true">Create Tour</a-button>
			</template>
		</a-page-header>

		<a-empty v-if="!tours.data || tours.data.length === 0" class="w-full mt-20" description="No tours available" />

		<div v-else class="grid gap-4 p-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
			<a-card v-for="tour in tours.data" :key="tour.name" hoverable :bordered="false"
				...
				<template #cover>
					<img class="h-64 sm:h-72 md:h-80 w-full object-cover" :src="tour.image || '/placeholder.jpg'"
						alt="Tour image" />
				</template>
			...
			<div class="mt-4">
				<FileUploader :fileTypes="['jpg', 'jpeg', 'png']" :multiple="false" @success="handleFileUpload"
					...
					<template #default="{ openFileSelector }">
						<div class="rounded overflow-hidden cursor-pointer flex flex-col items-center justify-center"
							@click="openFileSelector">
							<img v-if="newTour.image" :src="newTour.image" alt="avatar"
								class="w-full h-48 object-cover" />
							<div v-else class="h-48 flex flex-col items-center justify-center text-gray-500 text-sm">
								Upload Tour Image
							</div>
						</div>
					</template>
