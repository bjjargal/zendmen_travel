<template>
	<a-page-header class="!p-0 !mb-2" title="Lead" @back="() => $router.go(-1)">
		<template #tags>
			<a-tag v-if="lead?.isDirty" color="orange">Not saved</a-tag>
		</template>
		<template #extra>
			...
			<div class="border-b">
				...
				>
					...
					<a-form-item label="Country" name="country">
						<a-select
							v-model:value="lead.doc.country"
			...
			<div class="border-b">
				...
				>
					...
					>
						<a-space>
							<a-input-number
								v-model:value="lead.doc.group_min"
								:min="1"
								:max="20"
								addon-before="min"
							/>
							<a-input-number
								v-model:value="lead.doc.group_max"
								:min="1"
								:max="20"
								addon-before="max"
							/>
					...
					<a-form-item label="Comfort type" name="comfort_type">
						<a-select
							v-model:value="lead.doc.comfort_type"
					...
					<a-form-item label="Year" name="leadYear">
...
const createContact = async () => {
	try {
		await lead.createContact.submit();
