            ...
            <div class="border-b">
                ...
                <a-form class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2 !mt-2" layout="vertical">
                    ...
                    <a-form-item :colon="false" labelAlign="left" label="Group size">
                        <a-space>
                            <a-input-number v-model:value="oppertunity.doc.group_min" :min="1" :max="20" addon-before="min" />
                            <a-input-number v-model:value="oppertunity.doc.group_max" :min="1" :max="20" addon-before="max" />
                        </a-space>
                    </a-form-item>
                    ...
                    <a-form-item label="Year">
                        <a-checkbox-group v-model:value="oppertunityYear" :options="getYearOptions()" />
                    </a-form-item>
                </a-form>
            </div>
            <div class="">
                ...
                <a-form class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 !mt-2" layout="vertical">
                    ...
                    <a-form-item label="Business Cart">
                        <FileUploader :fileTypes="['jpg', 'jpeg', 'png']" :multiple="false"
                            @success="(file) => { oppertunity.doc.image = file.file_url }" class="border-none">
    ...
    <a-modal title="New Opportunity" v-model:open="oppOpen" okText="Create" @ok="createOpertunity">
        <a-form :model="oppForm" layout="vertical">
            <a-form-item label="Assignee" name="assignee">
                <a-select v-model:value="oppForm.assignee" :options="employeeOptions" placeholder="Select assignee">
            ...
            <a-form-item label="Deadline" name="deadline">
                <a-date-picker v-model:value="oppForm.deadline" format="YYYY-MM-DD" valueFormat="YYYY-MM-DD"
                    class="w-full" />
            </a-form-item>
